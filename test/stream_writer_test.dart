...
void main() {
  group('ExcelStreamWriter', () {
    test('creates valid xlsx with basic text and numbers', () {
      ...
      expect(
          (readSheet.rows[0][0]?.value as TextCellValue).value, equals('Name'));
      expect((readSheet.rows[0][1]?.value as TextCellValue).value,
          equals('Age'));
      expect((readSheet.rows[0][2]?.value as TextCellValue).value,
          equals('Score'));

      // Row 1: Alice
      expect((readSheet.rows[1][0]?.value as TextCellValue).value,
          equals('Alice'));
      ...
      // Row 2: Bob
      expect(
          (readSheet.rows[2][0]?.value as TextCellValue).value, equals('Bob'));
    ...
    test('handles null cells', () {
      ...
      expect((row[0]?.value as TextCellValue).value, equals('A'));
      // null cell may be null or have null value
      expect((row[2]?.value as TextCellValue).value, equals('C'));
    });
    ...
    test('handles XML special characters', () {
      ...
      expect((row[0]?.value as TextCellValue).value, equals('Tom & Jerry'));
      expect((row[1]?.value as TextCellValue).value, equals('<html>'));
      expect((row[2]?.value as TextCellValue).value, equals('a > b'));
    });
    ...
    test('shared strings deduplicates', () {
      ...
      expect((rows[0][0]?.value as TextCellValue).value, equals('hello'));
      expect((rows[1][0]?.value as TextCellValue).value, equals('hello'));
      expect((rows[1][1]?.value as TextCellValue).value, equals('hello'));
    });
    ...
    test('closeToFile writes to disk', () {
      ...
      try {
        ...
        expect(
            (excel['FileTest'].rows[0][0]?.value as TextCellValue).value,
    ...
    test('handles large number of rows', () {
      ...
      expect(
          (excel['Big'].rows[999][0]?.value as IntCellValue).value, equals(999));
    });
    ...
    test('handles custom styles', () {
      ...
      expect(
          (excel['Styled'].rows[0][0]?.value as TextCellValue).value,
          equals('Bold'));
    });
  });
}
